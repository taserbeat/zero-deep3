...
class Softmax(Function):
    ...
    def forward(self, x):
        ...
        y = np.exp(y)
        y /= y.sum(axis=self.axis, keepdims=True)  # type: ignore
        return y
...
def mean_squared_error_simple(x0, x1):
    ...
    diff = x0 - x1  # type: ignore
    y = sum(diff ** 2) / len(diff)  # type: ignore
    return y
...
def softmax_cross_entropy_simple(x, t):
    ...
    log_p = log(p)
    tlog_p = log_p[np.arange(N), t.data]  # type: ignore
    y = -1 * sum(tlog_p) / N  # type: ignore
    return y
...
def sigmoid_cross_entropy(x, t):
    ...
    p = sigmoid(x)
    p = clip(p, 1e-15, 1.0)
    tlog_p = t * log(p) + (1 - t) * log(1 - p)  # type: ignore
    y = -1 * sum(tlog_p) / N  # type: ignore
    return y
...
class BatchNorm(Function):
    ...
    def forward(self, x, gamma, beta):
        ...
        if x_ndim == 4:
            # (N*H*W, C) -> (N, C, H, W)
            y = y.reshape(N, H, W, C).transpose(0, 3, 1, 2)  # type: ignore
        return y
    ...
    def backward(self, gy):
        ...
        if x.ndim == 4:
            N, C, H, W = x.shape
            x = x.transpose(0, 2, 3, 1).reshape(-1, C)  # type: ignore
        mean = x.sum(axis=0) / batch_size  # type: ignore
        xc = (x - mean) * self.inv_std

        gbeta = sum(gy, axis=0)
        ggamma = sum(xc * gy, axis=0)
        gx = gy - gbeta / batch_size - xc * ggamma / batch_size  # type: ignore
        gx *= gamma * self.inv_std  # type: ignore

        if gy_ndim == 4:
            gx = gx.reshape(N, H, W, C).transpose(0, 3, 1, 2)  # type: ignore
...
from dezero.core import sub  # noqa
from dezero.core import rsub  # noqa
from dezero.core import mul  # noqa
from dezero.core import div  # noqa
from dezero.core import neg  # noqa
from dezero.core import pow  # noqa

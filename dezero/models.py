...
class VGG16(Model):
    ...
    def forward(self, x):
        ...
        # 整形
        x = F.reshape(x, (x.shape[0], -1))  # type: ignore
...
class BuildingBlock(Layer):
    def __init__(
        ...
        self.a = BottleneckA(in_channels, mid_channels, out_channels, stride, downsample_fb)  # type: ignore
        self._forward = ['a']
        for i in range(n_layers - 1):  # type: ignore
            name = 'b{}'.format(i + 1)
...
class BottleneckA(Layer):
    ...
    def __init__(self, in_channels, mid_channels, out_channels, stride: Union[int, Tuple[int, int]] = 2, downsample_fb=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return F.relu(h1 + h2)  # type: ignore
